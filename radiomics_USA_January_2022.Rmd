...
for (i in c("High","Low")) {
  for (j in c("Squamous cell carcinoma","Invasive adenocarcinoma" )){
     ...
     for (col in unique(vega$Pathway)){
      ...
      print(ggplot(df_res %>% filter(adj_p_val < 0.06) , aes(x = col, y = -log10(adj_p_val))) +
        geom_point() +
...
for (i in c("High","Low")) {
  for (j in c("Squamous cell carcinoma","Invasive adenocarcinoma" )){
    ids =pyrad %>% 
      ...
      print(ggplot(df_res %>% filter(adj_p_val < 0.10), aes(x = hal, y = -log10(adj_p_val), color = t.estimate)) +
        geom_point() +
        ggtitle(paste0(j," ","Volume(median): ",i,"\nssGSEA Hallmark correlation to radITH FDR adjusted pval"))+
        ylab("-log10(Adj.P)")+
        theme_minimal() + 
...
for (i in c("High","Low")) {
  for (j in c("Squamous cell carcinoma","Invasive adenocarcinoma" )){
      ...
      print(ggplot(df_res %>% filter(adj_p_val < 0.06) , aes(x = g, y = -log10(adj_p_val), color = t.estimate)) +
        geom_point() +
        ggtitle(paste0(j," ","Volume(median):",i))+
        ylab("-log10(Adj.P)")+
        theme_minimal() + 
...
print(ggplot(df_res %>% filter(t.p.value < 0.06) , aes(x = g, y = -log10(t.p.value), color = t.estimate)) +
  geom_point() +
