...
# Difference between radITH and Histology

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

ggplot(pyrad %>% filter(pathology != "Other"), aes(x = pathology,  y = radITH )) +
  geom_violin()+
  geom_beeswarm(cex = 2)+
  stat_compare_means()+
  theme_minimal() +
...
ggplot(pyrad %>% filter(pathology != "Other"), aes(x = pathology,  y = volume )) +
  geom_violin()+
  scale_y_log10()+
  geom_beeswarm(cex = 2)+
  stat_compare_means()+
...
cin_measures = c(cin_measures, "hrd_score")
...
tx_multi_reg_ge = as.data.frame(get(load("/Users/au589901/GenomeDK_local/CancerEvolution/phd/Datasets/TRACERx/RNAseq/RSEM_gene_name_TPM_df_20200204.RData")))

tx_multi_reg_ge = tx_multi_reg_ge[which(tx_multi_reg_ge$gene_id %in% bioITHgenes$Gene),]
colnames(tx_multi_reg_ge)
melted = reshape2::melt(tx_multi_reg_ge)
melted$sampleid = unlist(lapply(melted$variable, function(x){
  paste0(substr(str_split(x,"_")[[1]][2],0,3), "0",substr(str_split(x,"_")[[1]][2],4, 8))
}))
melted$tumor = unlist(lapply(melted$variable, function(x){
  str_split(str_split(x,"_")[[1]][4],"-")[[1]][1]
}))
melted$region = unlist(lapply(melted$variable, function(x){
  str_split(str_split(x,"_")[[1]][4],"-")[[1]][2]
}))

melted_t1 = melted %>% 
  filter(tumor == "T1",
         gene_id %in% as.character(bioITHgenes$Gene))
melted_t1$value = log2(melted_t1$value)


res = melted_t1 %>% 
  dplyr::group_by(sampleid, gene_id) %>%
  dplyr::summarize(sds = sd(value,na.rm = T)) %>% 
  dplyr::group_by(sampleid) %>%
  dplyr::summarize(rnaITH=mean(sds, na.rm=T))
...
tmp = merge(pyrad, res, by = "sampleid")

p1 = ggplot(tmp %>% filter(rnaITH < 1.5), aes(x = rnaITH, y = radITH, color = pathology)) +
  geom_point() + 
  theme_pubclean() +
  geom_smooth(method = "lm", se = F) +
  stat_cor()

p2 = ggplot(tmp %>% filter(rnaITH < 1.5), aes(x = rnaITH, y = radITH)) +
  geom_point() + 
  theme_pubclean() +
  geom_smooth(method = "lm", se = F) +
  stat_cor()
