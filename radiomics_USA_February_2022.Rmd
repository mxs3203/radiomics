...
```{r}
tmp = pyrad[pyrad$lung_specific_time/365 < 4, ]

tmp = tmp[which(!is.na(tmp$cens_lung_specific)), ]
...
feature_data <-mRMR.data(data = 
                           data.frame(target = as.ordered(pyrad$cens_lung_specific),
...
rf = randomForest::randomForest(x = tmp[,features_of_interest_cor], 
                                y = as.ordered(tmp$cens_lung_specific),
                                importance=T, 
                                nPerm = 100, 
                                ntree = 1000)
...
tmp = pyrad %>% 
  ...
  select(c(common_best_features,  "pathology", "cens_lung_specific", "volume", "diameter"))
tmp$response = make.names(tmp$cens_lung_specific)
...
test_tmp = tmp[-indx,]
   ...
   fit <- train(as.formula(paste("response~pathology+volume")), 
    ...
    p1 = ggplot(data.frame(response_var = test_tmp$response, 
              Prob = predict(fit, test_tmp, type="prob")[,2]), 
    ...
    auc = calc_auc(p1)[3][[1]]
    p1 = p1 + ggtitle(paste0("Test AUC: ",round(auc,4)))
    ...
    p3 = ggplot(data.frame(response_var = test_tmp$response, 
      ...
      theme(plot.title = element_text(size=12))
    auc = calc_auc(p3)[3][[1]]
    p3 = p3 + ggtitle(paste0("Test AUC: ",round(auc,4)))
    ...
    p5 = ggplot(data.frame(response_var = test_tmp$response, 
      ...
      theme(plot.title = element_text(size=12))
    auc = calc_auc(p5)[3][[1]]
    p5 = p5 + ggtitle(paste0("Test AUC: ",round(auc,4)))
    ...
    p7 = ggplot(data.frame(response_var = test_tmp$response, 
      ...
      theme(plot.title = element_text(size=12))
    auc = calc_auc(p7)[3][[1]]
    p7 = p7 + ggtitle(paste0("Test AUC: ",round(auc,4)))
